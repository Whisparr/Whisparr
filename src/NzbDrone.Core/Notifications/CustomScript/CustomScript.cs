...
namespace NzbDrone.Core.Notifications.CustomScript
{
    public class CustomScript : NotificationBase<CustomScriptSettings>
    {
        ...
        public override void OnGrab(GrabMessage message)
        {
            ...
            environmentVariables.Add("Whisparr_Movie_Tags", string.Join("|", movie.Tags.Select(t => _tagRepository.Find(t)?.Label).Where(l => l.IsNotNullOrWhiteSpace())));
            ...
            environmentVariables.Add("Whisparr_Release_CustomFormat", string.Join("|", remoteMovie.CustomFormats));
            environmentVariables.Add("Whisparr_Release_CustomFormatScore", remoteMovie.CustomFormatScore.ToString());
        ...
        public override void OnDownload(DownloadMessage message)
        {
            ...
            {
                environmentVariables.Add("Whisparr_DeletedRelativePaths", string.Join("|", message.OldMovieFiles.Select(e => e.MovieFile.RelativePath)));
                environmentVariables.Add("Whisparr_DeletedPaths", string.Join("|", message.OldMovieFiles.Select(e => Path.Combine(movie.Path, e.MovieFile.RelativePath))));
                environmentVariables.Add("Whisparr_DeletedDateAdded", string.Join("|", message.OldMovieFiles.Select(e => e.MovieFile.DateAdded)));
                environmentVariables.Add("Whisparr_DeletedRecycleBinPaths", string.Join("|", message.OldMovieFiles.Select(e => e.RecycleBinPath ?? string.Empty)));
            }
        ...
        public override void OnMovieAdded(Movie movie)
        {
            ...
            environmentVariables.Add("Whisparr_EventType", "MovieAdded");
            environmentVariables.Add("Whisparr_InstanceName", _configFileProvider.InstanceName);
            environmentVariables.Add("Whisparr_ApplicationUrl", _configService.ApplicationUrl);
            environmentVariables.Add("Whisparr_Movie_Id", movie.Id.ToString());
            environmentVariables.Add("Whisparr_Movie_Title", movie.MovieMetadata.Value.Title);
            ...
            environmentVariables.Add("Whisparr_Movie_ImdbId", movie.MovieMetadata.Value.ImdbId ?? string.Empty);
            environmentVariables.Add("Whisparr_Movie_TmdbId", movie.MovieMetadata.Value.ForeignId.ToString());
            environmentVariables.Add("Whisparr_Movie_AddMethod", movie.AddOptions.AddMethod.ToString());
            ...
            if (failures.Empty())
            {
                try
                {
                    var environmentVariables = new StringDictionary
                    {
                        ...
                        { "Whisparr_InstanceName", _configFileProvider.InstanceName },
                        { "Whisparr_ApplicationUrl", _configService.ApplicationUrl }
                    };
