        ...
        where TSettings : IProviderConfig, new()
    {
        ...
        protected ValidationFailure TestFolder(string folder, string propertyName, bool mustBeWritable = true)
        {
            if (!_diskProvider.FolderExists(folder))
            {
                ...
                {
                    DetailedDescription = string.Format("The folder you specified does not exist or is inaccessible. Please verify the folder permissions for the user account '{0}', which is used to execute Whisparr.", Environment.UserName)
                };
            }
            ...
            if (mustBeWritable && !_diskProvider.FolderWritable(folder))
            {
                ...
                {
                    DetailedDescription = string.Format("The folder you specified is not writable. Please verify the folder permissions for the user account '{0}', which is used to execute Whisparr.", Environment.UserName)
                };
            }
