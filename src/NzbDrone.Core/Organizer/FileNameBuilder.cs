...
namespace NzbDrone.Core.Organizer
{
    ...
    public class FileNameBuilder : IBuildFileNames
    {
        ...
        private void AddEpisodeTokens(Dictionary<string, Func<TokenMatch, string>> tokenHandlers, List<Episode> episodes)
        {
            ...
            {
                var femalePerformers = episodes.SelectMany(e => e.Actors.Where(a => a.Gender == Gender.Female).Select(a => a.Name)).Distinct().Take(4).ToList();
                return string.Join(", ", femalePerformers);
            };
            tokenHandlers["{Episode PerformersMale}"] = m =>
            {
                var malePerformers = episodes.SelectMany(e => e.Actors.Where(a => a.Gender == Gender.Male).Select(a => a.Name)).Distinct().Take(4).ToList();
