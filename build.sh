...
UpdateVersionNumber()
{
    if [ "$WHISPARRVERSION" != "" ]; then
        ...
        sed -i'' -e "s/<string>10.0.0.0<\/string>/<string>$WHISPARRVERSION<\/string>/g" distribution/macOS/Whisparr.app/Contents/Info.plist
    fi
}
...
PackageMacOSApp()
{
    ...
    rm -rf $folder
    mkdir -p $folder
    cp -r distribution/macOS/Whisparr.app $folder
