...
namespace NzbDrone.Integration.Test
{
    ...
    public class OpenApiFixture : IntegrationTest
    {
        ...
        [Test]
        public void every_json_request_body_should_be_required()
        {
            foreach (var (path, verb, operation) in GetOperations())
            {
                ...
                requestBody.TryGetProperty("required", out var required).Should().BeTrue("{0} {1} binds a body", verb, path);
                required.GetBoolean().Should().BeTrue("{0} {1} binds a body", verb, path);
            }
        }
