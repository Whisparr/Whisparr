...
namespace NzbDrone.Core.Test.UpdateTests
{
    public class UpdatePackageProviderFixture : CoreTest<GithubUpdatePackageProvider>
    {
        ...
        public void Setup()
        {
            ...
            Mocker.GetMock<IConfigFileProvider>().SetupGet(c => c.GithubOwnerRepo).Returns("whisparr/whisparr");
        }
        ...
        {
            UseRealHttp();
            Subject.GetLatestUpdate("v2", new Version(10, 0)).Should().BeNull();
        }
        ...
        {
            UseRealHttp();
            Subject.GetLatestUpdate("v2", new Version(0, 1)).Should().NotBeNull();
        }
