...
namespace NzbDrone.Core.Datastore
{
    ...
    public class ConnectionStringFactory : IConnectionStringFactory
    {
        ...
        public ConnectionStringFactory(IAppFolderInfo appFolderInfo, IConfigFileProvider configFileProvider)
        {
            ...
            switch (connectionStringType)
            {
                ...
                case ConnectionStringType.Sqlite:
                    ...
                    break;
                default:
                    throw new WhisparrStartupException("Unable to determine database connection string for type {0}.", connectionStringType.ToString());
            }
        }
        ...
        private ConnectionStringType GetConnectionStringType()
        {
            ...
            {
                throw new WhisparrStartupException("Postgres MainDbConnectionString is set but LogDbConnectionString is not. Both must be set or neither.");
            }

            if (isLogDBConnectionStringSet && !isMainDBConnectionStringSet)
            {
                throw new WhisparrStartupException("Postgres LogDbConnectionString is set but MainDbConnectionString is not. Both must be set or neither.");
            }

            if (isMainDBConnectionStringSet && _configFileProvider.PostgresHost.IsNotNullOrWhiteSpace())
            {
                throw new WhisparrStartupException($"Either both Postgres connection strings must be set, or the other Postgres settings must be set, but not both.");
            }
